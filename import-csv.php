...
<?php include('includes/helpers/parsecsv.php');?>
<?php
...
$q = 'SELECT lists.currently_processing, lists.prev_count, lists.total_records, login.timezone FROM lists, login WHERE lists.id = '.$listID.' AND login.app = lists.app';
$r = mysqli_query($mysqli, $q);
if ($r)
{
	while($row = mysqli_fetch_array($r))
...
// Reaching this far implies that there is indeed a CSV file. If now, the total_records = 0, then this
// file is being processed for the first time, and we need to save total_records first.
if ($total_records == 0) {
	$total_records = count(file($csvfile));
	$q = 'UPDATE lists SET total_records = '.$total_records.' WHERE id = '.$listID;
...
if(!$current_processing)
{
	...
	// if(!isset($_GET['offset'])) set_prev_count(); TODO
	...
	foreach ($csv->data as $key => $line)
	{
		...
		$q = 'SELECT email FROM subscribers WHERE list = '.$listID.' AND (email = "'.$linearray[0].'" || email = "'.trim($linearray[1]).'")';
		$r = mysqli_query($mysqli, $q);
		...
		// {
			if($key+1 == ($total_records - $prev_count))
			{
				...
				// Update prev_count to prev_count + $key, and stop for this run
				$prev_count = $prev_count + $key;
	...
	// If we have gracefully reached the end of this loop we should reset things.
	//set currently_processing to 0
	$q = 'UPDATE lists SET currently_processing=0, prev_count=0, total_records=0 WHERE id = '.$listID;
	mysqli_query($mysqli, $q);
...
function set_prev_count($count = NULL)
{
	global $listID;
	global $mysqli;
	if ($count == NULL) {
