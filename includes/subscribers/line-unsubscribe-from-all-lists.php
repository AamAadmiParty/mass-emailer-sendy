...
{
	//show error msg
	header("Location: ".get_app_info('path').'/unsubscribe-from-all-lists?i='.$app.'&l='.$listID.'&e=2'); 
	exit;
}
...
{
	$q = 'UPDATE subscribers SET unsubscribed = 1 WHERE email = "'.trim($line_array[$i]).'" AND userID = '.$userID;
